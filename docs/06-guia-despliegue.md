...
| El despliegue sale en verde pero faltan Functions | La primera vez que se despliega en un proyecto nuevo, todas intentan crear a la vez el bucket `gcf-v2-sources` y chocan con un 409. `firebase deploy` avisa con un ⚠ y **sale con código 0** | Volver a desplegar: el bucket ya quedó creado. El paso «Verificar que están TODAS las Functions» del pipeline lo detecta ahora en lugar de dejarlo pasar |
